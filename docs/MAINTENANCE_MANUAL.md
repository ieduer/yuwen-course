...
Change (released and accepted 2026-09-26):
- A saved reply is reused only if it passes the same scoring contract as foreground scoring (`usableEvaluationReply`: shared `extractJsonObject` + `normalizeOpenStudyGuideAssessment`). Invalid replies stay in `learning_evaluation_replies` and their `assistant.reply` events; nothing is deleted or rewritten.
- `invalid_ai_assessment` now queues the job for a fresh model call (1/5/15-minute backoff) while fewer than four lifetime calls exist, recording `ai.failure` (reason `invalid_ai_assessment`, parent = the invalid reply's event) and `ai.retry` (`executionKind: apis_call`); `last_error_class` is `invalid_reply`. With four calls spent it blocks as before.
- Reconciliation for jobs blocked by the old rule: the only path to `blocked` + `last_error_class='reply_saved'` was an invalid assessment. Each scheduler tick requeues up to two such jobs whose latest reply is invalid and whose call budget remains, recording `ai.retry` with `retryKind: reconciliation` (key `reconcile-<epoch>`, parent = the `evaluation.blocked` event, or the reply event). Jobs with a spent budget stay blocked.
- Release order: Pages first (reuse and retry rules), then `yw-evaluation-jobs` (reconciliation pass). No schema, binding or configuration change.

Release (owner approval "批准 YW 對帳發布"): Pages `6037b165-e4c5-43d2-a66b-5a03cce55816` (12:01:15Z) through the registered Pages adapter, artifact 1232 files that differ from the accepted `edbb05d2` artifact only in the three program files and the release marker; rollback `edbb05d2` / 367e8f6. Scheduler `yw-evaluation-jobs` version `37b94ddc-60e3-4828-9270-8b91016e511d` (deployment `dd2ec637`, 12:07:48Z) with the same activation vars and inherited machine secret; rollback `4db3adc4` / 6049d87. Natural acceptance: on its first tick the scheduler reconciled the blocked lesson-1488 job (12:08:07Z): one fresh call answered by gemini-3.6-flash (386 characters), `evaluation.result` and completion sync recorded, the 2-character invalid reply preserved; open jobs 0 and the Pulse alert state `recovered:ok`. Evidence: `/Users/ylsuen/CF/reports/operations/yw-reconcile-20260926/`.
...
## 2026-09-25 — detailed-learning recorder candidate

See [LEARNING_RECORDER_BRIDGE.md](LEARNING_RECORDER_BRIDGE.md). The Web candidate
reads complete immutable source facts only for the authenticated owner, verifies
canonical hashes, and durably hands them to the accepted UC `record_only`
recorder. Its private full-content observations are separate from the unchanged
minimized scoring projection. No S2S logger, source re-evaluation, history
backfill, migration, shared contract, App pointer or background Worker change.
Current production background and alerts are true; earlier disabled/unshipped
sections below predate the completed source6049/Pagesf8808e83 rollout and window
handoff. Natural completion acceptance still belongs to the original repair
task; this new candidate is not production acceptance.
...
## Signed background execution candidate — 2026-09-25

PR62 uses `/api/internal/evaluation-jobs/execute` with a separate 32-byte HMAC secret (`YW_EVALUATION_MACHINE_SECRET`) in Pages and `yw-evaluation-jobs` only. Existing APIS credentials remain unchanged. The scheduler retains D1/Pulse bindings, no APIS/Queue bindings, and no public route. Machine bodies contain only opaque job id/lease epoch. Enforce 60-second freshness, nonce uniqueness, snapshot ownership and D1 epoch-start CAS; unauthorized calls return 404. Unknown delivery cannot automatically start another provider call; saved replies recover from the journal. Workers requires redirect mode `manual`, followed by explicit non-200 rejection.

Release is pending: Worker disabled, backed-up additive 0009 without historical enrollment, Pages exact configuration and guarded release/authenticated readback, separate Pulse release/natural readback, then background activation/natural completion. Preserve all forward data on rollback. Recorder schema remains bdfz-learning-operation-v1; the later authenticated bridge supplies scope. See PROJECT_STATE and workspace reports/operations/yw-http503-20260925/review-20260925/PENDING_RELEASE_GAPS.md for the current transaction.

## 2026-09-25 recorder source preservation (unpublished)

The unshipped0009 additionally creates learning_evaluation_events. Each private source fact has a permanent `yw:` ID derived from source/kind/stable ledger key (canonical tuple SHA-256 when required by the12–180 character rule), immutable original event time, parent ID, canonical full JSON, SHA-256 and UTF-8 bytes. Conflict/update/delete triggers prevent silent overwrites. Content is stored inline, so a future bridge need not trust mutable snapshot pointers. The bridge owns authentic UC scope and exact central acknowledgement; this leaf never computes scope, impersonates a browser or submits to a second logger. The event schema is the recorder successor's bdfz-learning-operation-v1 with scope intentionally absent until authenticated bridging.

Submission event and original input save together; each actual send intent precedes model I/O and includes stable call-ledger identity, epoch and request ID. Preserve all typed failures, planned retry decisions and local-only recovery decisions separately from job state. Full AI answer and exact successful APIS response JSON save before the secondary assistant.reply event; an event write failure retains the reply and later reconstructs its original time without another model call. Invalid replies are preserved, never converted to zero grades and never reused; since 2026-09-26 the job asks the model again within its lifetime call budget and blocks only when that budget is spent. Normalized source result/model suggestion remain separate; result, grade, pending completion and score outbox are one fenced transaction. Queue acknowledgement is pending, not central accepted. Central receipt reads and typed failures have their own immutable observations.

Production stays PR59ffa3689/ff11ac3d. Source facts are private student data and must be included in the additive backup/restore contract. Code rollback preserves every original, reply, event and score/outbox row. Neither0009 nor this event code has been published. Existing background credential architecture and independent stale-Cron monitoring blockers remain.
...
Truthful assessment failure/cooldown and missing-rubric display are in the exact live artifact;40 focused tests and14 preview regressions pass. No AI-graded answer was submitted. Intermittent assessment availability remains unresolved while the separately prepared APIS recovery cannot complete authenticated qualification: the user-authorized ceiling is16 provider attempts, currently0 observed. Local credential search found no usable YW caller value; the prior paired-rotation receipt records temporary credential cleanup. Natural YW18:46 and UC19:02 Beijing recovery events preceded candidate traffic and are not release acceptance.

Rollback remains Pages `0670bd26-e2d8-494b-bef0-97ebce877ce5`; retain D1/Queue data. Full Pages projection, bindings, preview logger and App pointer/tree are unchanged. The fixed11:41:40.702–11:51:40.702Z host window has6sampled requests, all200; this limited aggregate is not AI grading acceptance. The independent UC candidate later failed NH heartbeat acceptance and was rolled back to0c252890; YW Web behavior and its own release acceptance remain unchanged. Shared receipt and final scope are recorded in the single live report: `/Users/ylsuen/CF/reports/operations/yw-uc-incident-20260915/serial-release/REPORT.md`. Earlier preparation entries below are historical.
...
Current Pages **0670bd26-e2d8-494b-bef0-97ebce877ce5**, runtime source **5ed3e1b7262c510de0434d4d690465f04931f7d4** (PR39). Existing v2 executor **1de239210af218ca8b94e726744dd76fee6a31a0** (ops PR40) accepted the deployment after real authentication and post-release telemetry. Immediate rollback is **c66da540-07dc-4f2f-9adf-dbf27fdd251d**, sourcea9d7f654. Artifact1224files/164473235bytes, digestc4026ac38605f1c42b270d87c2aae72423ae625e9bbe04cdf9146ffc03c29566; full config7c6d35e2feb54a7aa1965e9551b253755c315658caaea6b149d9a7753117def6. Logger36279914, bindings, credentials, D1/Queue and native276objects/pointer are unchanged.

Four exact CText references now show existing reviewed1024×640 WebP first screens dated2026-08-11, with explicit not-full-text captions and original URLs, including Liji #n9662. Real owner flow saved3marks and submitted initial reading06:50:34Z; My contained the corresponding non-scoring initialReadingSubmitted and5resourceOpened events. Both source and My UI read back after reload at06:59:58Z. All4compact,4enlarged and4reloaded images decoded, with0CTextproxyrequests. This verifies the actual preview/initial-reading path; screenshots do not restore the publisher's full-text service.

Fixed06:44:10–07:10Z telemetry: private logger78/78 retained ABR1 events,14requests withterminals,9HTTP200/5non-CText403/0CText/0HTTP521. Zone133adaptive aggregate requests,0HTTP5xx/521; sampleintervals1–4, so this is not133raw requests. Prior4storedCText521s are absent from the exercised repaired path. Five other preview403s are existing-policy redirect denials after upstream301/302/307, not raw upstream403. One learning/interactions422 at07:00:24.953Z remains unexplained. Those defects and fullYW/AI/fleethealth remain independently open; do not claim overall completion.

Acceptance authority: `/Users/ylsuen/CF/_meta/reports/operations/.my-architecture-serial7-20260909/yw-521-production-acceptance.json` (SHA25643b04bf8dc33b200020dfbb8bb084323eee4e9a3dddc06cc20c3374578cdd4b4); exact accept output `yw-521-production-accept-output.log`; final telemetry `yw-521-postrelease-freeze.json` supersedes the initial metadata-ID/null interpretation. Journal `yw-ctext-release-control/journal.jsonl` is **accepted**, SHA2564c56af5d391524a0b8b17e1fa7335e3c401ee37a118a061dcfef5612e382145c. Ownercodex-my-architecture-serial7.

Rollback procedure: use clean registered worktrees pinned to the exact YW5ed3e1b and executor1de2392 commits, Node24.18.0, then run `node foundation-yw-pages-leaf-release.mjs rollback --ops-dir <OPS_WORKTREE> --yw-dir <YW_WORKTREE> --executor-sha 1de239210af218ca8b94e726744dd76fee6a31a0` from the opsworktree. The existing v2 rollback explicitly accepts an accepted journal, verifies the exact owned candidate, and restores c66da540 with full config/Web readback. Never replay deploy or edit the immutable journal; preserve all forward student evidence. Later documentation commits are not deployed runtime authority.

Resource location/restore: source and four WebP inputs remain Git-backed in ieduer/yuwen-course; executor inieduer/cf-ops-scripts. Restore into absent manifest-registered worktrees using `git worktree add --detach <ABSENT_YW_WORKTREE> 5ed3e1b7262c510de0434d4d690465f04931f7d4` and equivalent opscommit1de239210af218ca8b94e726744dd76fee6a31a0; verify exactHEAD/clean status before rollback. Existingone-usejournal and sanitizedevidence remain retain_hot under E7, review2026-09-16, until superseding recovery acceptance; review date is not deletion authority. Reproducible taskworktrees/builds remain root-owned for exact cleanup. No new dataset/archive/credential is added. CAPABILITY_FIT=no-new-capability.

The older dated states below are historical where this accepted release supersedes them. `/api/learning/health` drains the outbox and `/api/preview` may issue a CText loginPOST; neither is a read-only health probe.
