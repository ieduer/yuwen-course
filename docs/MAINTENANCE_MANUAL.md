## 2026-09-11 18:00 北京 interaction-check 503 incident readback

Read-only diagnosis preserves Pages0670bd26/source5ed3e1b7 and immediate rollbackc66da540/sourcea9d7f654. Five503 observations all occurred at09:45Z on `/api/interaction-check`; original alert5/141 is retained, while the09:44–10:00Z adaptive re-query is5/146. Later09:46–10:13:50Z has212adaptive observations/0HTTP5xx, not exact raw request counts or final recovery acceptance. Home, manifest, taxonomy and read-only Reading health returned200 at10:12Z.

APIS remains95a1ec95@100/deployment6e77d0b3 (10:10Z readback), withslot6 excluded. Complete09:40–09:49Z persisted fetch logs have91rows and7identifiedYW HTTP200/0HTTP503; this does not support assigning today's five failures to APIS transport. D1 aggregate09:40–09:50Z has8evaluator calls and7captured submissions nowcompleted; one09:45submission had a prior release and successfulretry. All7formal interactions haveevaluation andcentral acceptedreceipt. This is existing-source/central readback, not a new authenticated post-reload acceptance. No learner data were modified or raw records read.

The precise failing branch is unresolved: strict assessment parsing/validation, transient identity/store error and15second evaluator cooldown can return503 locally. Persisted per-response error code and cross-service correlation are missing. The191/189manifest-taxonomy difference is intentional: two retired/nonstudent entries have noformal items and normalUI does not submit them. Do not add invented taxonomy or weaken assessment to suppress this alert.

No rollback: current and prior `site/_worker.js` have identical blob `bab96a2072c7555ca6f7a4b73bca856abce91ad4`; reverting Pages cannot fix these branches and removes acceptedCText mitigation. Preserve forward D1/Queue/UC evidence and credentials. APIS is owned by its separate recite repair task and remains read-only here. Next bounded YW repair should capture finite branch codes/request hashes without private payloads, then fix the demonstrated cause and verify real source→My→reload. Student-usable course coverage was not expanded by this incident audit.

Evidence and exact query/rollback boundaries: `/Users/ylsuen/CF/reports/operations/yw-http503-20260911T1000/README.md`. Source/data/runtime/bindings/App pointer unchanged. CAPABILITY_FIT:no-new-capability. Evidence retain_hot for active incident follow-up, ownercodex-yw-p0-20260911, review2026-09-18; no new remote resources or hydrate inputs. Recover documentation from branch `codex/yw-http503-incident-20260911` in existing repository; no checkout/build needed for read-only `git show`. Pre-existing dirty manuals and other files remain intact; one-off diagnostic/index are removed at closeout, runtime manifest20260911-yw-http503-1000.
...
Persisted ABR1/count-matched readback is complete:21rows/twoHTTPterminals. Exactly one source-request hash joins the failed probe to candidateae7 and slot39: terminal3.5-lite started with20384msremaining, timeout19884ms, then timed out at19884ms. The baseline95a separately completed slot40 in5923ms. These are different requests: this neither proves a key39permanent defect nor authorizes isolating it. The original8s ceiling can reject9sresponses locally, but extending it failed to recover this live request; do not ship or claim error reduction from this candidate. Source73tests, controller/probe38tests and readback20selftests passed, all separately from failed production acceptance.

EvidenceE7: apis-terminal-deadline-upload-result.json, apis-terminal-deadline-plan.json.journal.jsonl, apis-terminal-deadline-stage-1-probes.json, apis-terminal-deadline-provider-forensic.json, apis-terminal-deadline-context-closeout.json. Uploaded bundle194177B/a13c0e5a is verified; all71bindings/51Store refs and existing runtime preserved. Upload annotation-only difference was proved by exact preimage hash reconstruction. All controllers are terminal, their ownership locks released; only the newAPIS tab88222580 was closed, My/Gydb tabs retained. Never replay this terminal controller or its immutable plan.

YW Pages0670bd26/runtime5ed3e1b7 remains unchanged. Original521was absent in the later1060-observation adaptive window, but6study-guide-attempt503observations and five explicitYWAPIS503traces remain unaccepted; no cross-service join of all6. Default3.6/provider recovery, realYWassessment→My→ordinaryreload, and onlineerrorreduction remain open. RootMy/fleet scope, KZ typed-rate and MyreaderUI are independent unresolved purposes. Rollback authority for subsequentAPIS repairs is95a, preserving slot6 isolation. No-new-capability; source/evidence retain_hot underrootowner/review2026-09-16; rootmanifest owns runtime cleanup.
