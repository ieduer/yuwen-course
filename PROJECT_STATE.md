...
## 2026-09-25 signed background evaluation candidate (PR62, not published)

The owner approved an independent 32-byte HMAC machine credential held only by Pages and the scheduler. Existing Pages APIS credentials stay unchanged. The scheduler has no APIS or Queue binding; it claims D1 jobs and sends only opaque job id/lease epoch to the fixed Pages machine endpoint. Timestamp freshness, nonce uniqueness, snapshot owner/resource checks and D1 epoch-start CAS fence replay and duplicate provider starts. Unauthorized calls uniformly return 404. Redirects and malformed/oversized receipts become unknown outcomes without model retry.

Local contract/recovery/replay tests: 120 passed. Native workerd foreground/scheduler/D1 and named health RPC passed; models are local fixtures. Full precontent checks passed across the original run, Chrome touch check and remainder run. Brave synthetic touch gestures failed on unchanged UI; Chrome passed the same assertions. No production mutation or new real model validation occurred. Paired Worker-disabled/bootstrap, backed-up additive 0009, Pages configuration and guarded publication, Pulse natural schedule, then background activation and natural record readback remain required. Baseline remains ffa368975cba988b0427a86046a27efe986a6859 / ff11ac3d-2cd1-4aca-beb0-c2dc3e0cfd06.

YW ownership stays in task 01a0d84f-8f1c-7673-af8b-327318976041 until accepted; then hand to recorder task 01a0d9ec-8876-72f1-bc40-870add9fde19. The actual existing Pulse cron is every 15 minutes, not the instruction's assumed 5 minutes; preserve existing scheduling and report the difference. Evidence: workspace reports/operations/yw-http503-20260925/review-20260925/.

## 2026-09-25 recorder-aligned source facts — candidate only

The recorder successor confirmed the private append-only source plan and `yw:` event IDs. Candidate0009 now stores immutable source events for answer.submit, evaluation.pending, per-call ai.request/failure/retry, assistant.reply, evaluation.blocked/result and completion.sync.*. Full content is inline with canonical SHA-256 and UTF-8 byte counts; no mutable content references, UC scope or recorder transport are fabricated. Original input and submit event are atomic; requests persist before APIS I/O; the irreplaceable full reply/raw success body persists before its recoverable event projection; result/grade/outbox commit atomically under the existing epoch fence. Unknown model/version/provider counts remain explicit. Legacy Queue behavior outside durable jobs is preserved.

179 focused tests (including98 source-contract cases), a real dual-workerd/D1 synthetic lifecycle, full precontent, formal1229-file build and scheduler dry build pass after this event addition. Formal aggregate SHA-256:44f009425535435743b147062713bc4a6228d90a690b3a026a7727a58af50d6e. These are local candidate checks, not production or real-user acceptance. The exact UC65be121 validator fixture is pinned in scripts/fixtures/uc-learning-operation-journal-65be121.js. YW ownership stays here until pending publication is accepted, then transfers explicitly to01a0d936-c738-7c80-b218-24af851e0cc7 for the authorized backend bridge. APIS caller execution architecture and independent Pulse ownership remain unresolved; no production migration/config/release or new model validation occurred.

## 2026-09-25 durable evaluation candidate — not published

Candidate base is accepted PR59 runtime ffa368975cba988b0427a86046a27efe986a6859 (Pages ff11ac3d), including the retired PR60 pin; documentation from main02951cdd is preserved. Production blocker: the existing YW APIS caller secret is unreadable and absent from the approved local store; direct provisioning to the new Worker is not yet possible. No credential rotation is authorized by the observation-only APIS scope. A signed Pages execution bridge is designed for owner choice, not implemented. New submissions can durably retain original input and every AI reply/model revision, return owner-scoped202 pending, and complete through a separately gated YW scheduler. Both activation flags default off; no production migration, background Worker, config change or student data write has occurred. Historical31retryable submissions are not automatically enrolled.
